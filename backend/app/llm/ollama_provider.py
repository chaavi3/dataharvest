...
def _parse(raw: str) -> list[dict]:
    ...
    try:
        ...
        logger.error("Ollama LLM returned non-JSON: %s", text[:200])
    return []


class OllamaProvider(LLMProvider):
    """Talks to a local Ollama instance via its OpenAI-compatible API."""

    def __init__(self, base_url: str = "http://localhost:11434", model: str = "llama3", **_kw: str):
